...
## Один скорер и вопросы, которых он не видел

`open-jev-base` — та же механика, наведённая сразу на 40 задач: поточечный кросс-энкодер, который
оценивает одну тройку `(вопрос, вариант, текст)` и возвращает вероятность, так что K и формулировки
вариантов задаются в момент запроса, а не в архитектуре. Веса:
**[sshalimov04/open-jev-base](https://huggingface.co/sshalimov04/open-jev-base)**. Всё было
пререгистрировано до запуска ([`docs/prereg/base-v2.md`](docs/prereg/base-v2.md)); таблицы —
[`docs/experiments.md`](docs/experiments.md#b2--open-jev-base-converged-one-unseen-question-claim-survives-the-diversity-ablation-is-voided-by-its-own-stop-rule).

- **Пререгистрированное правило выполнено на трёх наборах, которые здесь никто не открывал.**
  yahoo-topics acc **0,528** (случайный уровень 0,088, учитель 0,718), sst5 MAE **0,775**
  (случайный 1,152, учитель 0,493), ru-inappropriate AUROC **0,619** (случайный 0,500, учитель
  0,891): лучше случайного на всех трёх, все доверительные интервалы не накрывают 0, и это
  **70 / 57 / 30 %** от отрыва учителя над случайным уровнем — при **200 золотых калибровочных
  строках на задачу**. Это и есть всё утверждение целиком: три вопроса, три источника текста, один
  сид. Слов *универсальный* и *любой вопрос* в нём нет.
- **Он проигрывает каждому специализированному ученику.** Единственное исключение — схлопнувшаяся
  16-классовая голова `m2w-element` (0,136 против 0,059). В остальном: −13,8 пункта на kinopoisk,
  −37,8 на banking77, −55,2 на swde-field, +0,16 MAE на georeview, −0,027 AUROC на toxic — все
  интервалы не накрывают 0.
- **Обучение до сходимости сделало перенос хуже**, а не лучше: на 5 из этих 6 отложенных задач
  сошедшаяся модель уступает 45-минутным, которые были до неё. А без золотых калибровочных строк
  sst5 падает до 0,48, и тогда пререгистрированное правило провалилось бы на 2 наборах из 3.
- **Абляция разнообразия — void.** Она прошла все три части своего порога и всё равно не считается:
  2 контрольных сида из 3 так и не сошлись, а пререгистрация такое сравнение запрещает. Ничто здесь
  не говорит, что 27 маленьких задач в смеси что-то дали:
